...
<p>vielen Dank für deine Buchung bei himmlischberaten.de. Deine Buchungsdaten im Überblick:</p>
...
<table cellpadding="10px" border="1" width="750px" style="border-collapse: collapse;">
	...
	<tr>
		...
		<td style="vertical-align: top"><b>Zahlmethode:</b> {{$transaction->payment_type=='CARD'?'Kreditkarte':$transaction->payment_type}}</td>
	</tr>
	<tr>
		<td><b>Dein Beratungstermin:<br></b>Datum:  {{$appointment->start->format('d.m.Y')}} <br>Startzeit:  {{$appointment->start->format('H:i')}} Uhr<br>Dauer: {{$appointment->end->diffInMinutes($appointment->start)}} Minuten<br>
		@if($appointment->mode=='offline')
			Ort: offline:<br>{{$coach->street}}<br>{{$coach->post_code}} {{$coach->place}}</td>
		@else
			Ort: online<br></td>
		@endif
		<td><b>Dein Berater:<br></b>{{ $coach->coach_company}}<br>{{$coach->first_name}} {{$coach->last_name}}<br>{{$coach->street}},{{$coach->house_no}}<br>{{$coach->post_code}} {{$coach->place}}<br><a href="{{route('coach-detail',['name'=>$coach->first_name.'-'.$coach->last_name,'id'=>$coach->id])}}">Zum Profil</a></td>
	</tr>
</table>
<br>
<br>
...
@endcomponent
@endif
<p>Die Rechnung zu deiner Beratung findest du im Anhang dieser E-Mail. </p>
