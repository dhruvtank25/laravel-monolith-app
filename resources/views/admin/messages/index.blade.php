...
@section('styles')
    ...
    <link rel="stylesheet" href="{{ asset('backend/css/message.css') }}">
@endsection
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
        ...
        <div class="modal-body coach_msg_body">
            <form method="post" action="{{url('admin/new-conversation')}}" id="messageFrm">
                @csrf
                @if($coach_id)
                    <input type="hidden" name="coach_id" value="{{$coach_id}}" required>
                @endif
                <input type="hidden" name="conv_id" id="conv_id" value="0" required>
                <input type="hidden" name="attachments" id="attachment" value="">
                ...
                <div class="col-md-12 accordion coach_msg_accord" id="accordionExample">
                    @foreach($threads as $thread)
                        ...
                        <div class="card {{$unread>0?'unreadmsg':''}}">
                            ...
                            <div id="thread_{{$thread->id}}" class="collapse" aria-labelledby="heading_{{$thread->id}}" data-thread-id="{{$thread->id}}" data-parent="#accordionExample">
                                <div class="card-body ">
...
<script>
    $(document).ready(function() {
        ...
        $("div#dropzone").dropzone({
            addRemoveLinks: true,
            url: "{{ url('admin/upload-attachments') }}",
        ...
        $('.collapse').on('show.bs.collapse', function () {
            ...
            // Total unread count
            var total_unread =  $("#header").find('.unread-message-count').text();
            ...
            .done(function(data) {
                ...
                if(data.success=='true') {
                    ...
                    $("#header").find('.unread-message-count').text(new_unread);
                }
            })
            .fail(function(error) {
                console.log("error");
