...
class MessageController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $coach_id = isset($request->coach_id)?$request->coach_id:'';
        $threads = $this->messageRepo->getConversations($user_id, $coach_id);
        return view('admin.messages.index', compact('page_title', 'threads', 'user_id', 'coach_id'));
    }
    ...
    public function storeConversation(Request $request)
    {
        ...
        if(!isset($inputs['conv_id']) || $inputs['conv_id']==0) {
            ...
            $data_arr = array('sender_id' => $admin_id, 'receiver_id' => $coach_id, 'subject' => $subject);
            ...
            if(!$conversation || ($conversation->user_one!=$admin_id && $conversation->user_two!=$admin_id))
                abort(401);
        }
        if($conv_id!=0) {
            // Save new message
            Talk::setAuthUserId($admin_id);
    ...
    public function markThreadSeen($thread_id)
    {
        ...
        $updated_count = $this->messageRepo->makeUnseenMessageSeen($thread_id, $admin_id);
        if($updated_count)
